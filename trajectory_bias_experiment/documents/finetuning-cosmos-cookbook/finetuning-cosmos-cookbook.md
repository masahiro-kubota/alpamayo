...
**必要なGPUメモリ (VRAM): 24GB以上** (RTX 3090/4090, A10G等)

> **メモリ計算の根拠**:
> *   **モデル重み**: 約20GB (10Bパラメータ × 2バイト[bfloat16])
> *   **学習オーバーヘッド**: 約2~3GB
>     *   通常、学習には重みの数倍のメモリが必要ですが、今回は **VLMバックボーンを凍結 (Freeze)** して推論モードで動かすため、巨大なVLM部分の勾配や中間データを保存しません。
>     *   学習するのは軽量なDiffusion Headのみであるため、合計24GBのVRAMに収めることができます。
